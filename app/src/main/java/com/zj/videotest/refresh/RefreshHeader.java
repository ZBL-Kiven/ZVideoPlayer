...
public class RefreshHeader extends SimpleComponent implements com.zj.views.list.refresh.layout.api.RefreshHeader {
    ...
    public void onMoving(boolean isDragging, float percent, int offset, int height, int maxDragHeight) {
        ...
        if (!isReleaseMoving) {
            if (percent < 0.4f) return;
        } else {
            percent += 0.4f;
        }
        float absPercent = Math.min(Math.max(percent - 0.4f, 0f), 1.0f);
    ...
    public void onInitialized(@NonNull RefreshKernel kernel, int height, int maxDragHeight) {
        ...
        standingLookupAnim.addUpdateListener(animation -> upDateAnimate(animation.getAnimatedFraction()));
        standingLookupAnim.addListener(new Animator.AnimatorListener() {
            ...
            public void onAnimationCancel(Animator animation) {
                ...
                iv.setRotationX(0);
                animRepeatCount = 1;
                if (loadingPop != null) loadingPop.curFraction = 0;
                postInvalidate();
            }
            ...
            public void onAnimationRepeat(Animator animation) {
                animRepeatCount++;
                if (loadingPop != null) loadingPop.setIsJump(animRepeatCount % 2 != 0);
            }
        });
    }
    ...
    private static class LoadingPop {
        ...
        private void randomAssets() {
            ...
            this.tx = (int) (DPUtils.dp2px(r.nextInt(20)) + (isLeft ? 1 : 0) * (maxSize / 2f) + 0.5f + maxSize);
        }
