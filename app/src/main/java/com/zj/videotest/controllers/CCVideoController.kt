    ...
    override fun onImgGot(path: String, type: ImgLoader.ImgType, tag: String, e: Exception?) {
        ...
        when (type) {
            ImgLoader.ImgType.IMG -> {
                ...
                Glide.with(thumb).asBitmap().load(path).into(thumb)
//                Glide.with(background).asDrawable().load(path).centerCrop().thumbnail(0.35f).transform(RenderScriptBlur(background.context, 12)).into(background)
            }
            ImgLoader.ImgType.GIF -> {
                Glide.with(thumb).asGif().load(path).into(thumb)
