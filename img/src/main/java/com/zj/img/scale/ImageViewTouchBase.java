...
import com.zj.img.scale.easing.ScaleEffect;
import com.zj.img.scale.graphics.FastBitmapDrawable;
import com.zj.img.scale.utils.IDisposable;
    ...
    public void setImageDrawable(final Drawable drawable, final Matrix initial_matrix, final float min_zoom, final float max_zoom) {
        ...
        if (viewWidth <= 0) {
            mLayoutRunnable = new Runnable() {
                @Override
                public void run() {
                    ImageViewTouchBase.this.setImageDrawable(drawable, initial_matrix, min_zoom, max_zoom);
                }
            };
            return;
        }
