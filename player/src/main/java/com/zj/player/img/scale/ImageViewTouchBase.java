...
import com.zj.player.img.scale.graphics.FastBitmapDrawable;
import com.zj.player.img.scale.utils.IDisposable;
...
abstract class ImageViewTouchBase extends AppCompatImageView implements IDisposable {
    ...
    public void setImageDrawable(final Drawable drawable, final Matrix initial_matrix, final float min_zoom, final float max_zoom) {
        ...
        if (viewWidth <= 0) {
            mLayoutRunnable = () -> ImageViewTouchBase.this.setImageDrawable(drawable, initial_matrix, min_zoom, max_zoom);
            return;
        }
